...
func main() {
	flag.Parse()
	logger := log.With(
		log.NewStdLogger(os.Stdout), //newZapLog(),
	...
	log.Infof("load bootstrap conf:[%+v]", bc)
	log.Infof("swagger url: %+v", "http://127.0.0.1:8000/q/swagger-ui/")
...
func newZapLog() *zaplog.ZapLogger {
...
func watch(c config.Config, key string) {
	if err := c.Watch(key, func(key string, value config.Value) {
		log.Infof("config(key=%+v) changed: %+v\n", key, value.Load())
	}); err != nil {
		panic(err)
	}
}
